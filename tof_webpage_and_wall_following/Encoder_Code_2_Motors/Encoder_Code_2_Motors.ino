...
void PIDcontrol() {
  ...
  if (dt >= 0.100) {//RUN PID AT 500 MS, BUT CAN CHANGE
    ...
    for (int i = 0; i < 2; i++) { 
      ...
      float current_rpm = ((current_count-previous_count[i])/(float)count_per_revolution)*(60.0/dt);
      float individual_target = rpm_desired * motor_direction[i];
      float error = individual_target - current_rpm; //CALCULATE ERROR
      float output = Kp*error; // P OUTPUT
      integral[i] = constrain(integral[i]+(error * dt), -110, 110); //Integral tuning
      ...
      if (control_output > 0) {
        motor_direction[i] = 1;
        motor_speed[i] = constrain(control_output * (resolution / 110.0), 0, resolution);
      ...
      motor(i);
      Serial.printf("MOTOR %d | Target RPM: %.1f | Current RPM: %.1f | Dir: %d | PWM: %.0f\n", 
...
void handleSpeed() {
  ...
  Serial.printf("RPM set to: %.1f, PID enabled\n", rpm_desired);
    ...
    for(int i=0; i<2; i++) {
      motor_speed[i] = map(constrain(rpm_new, 0, 110), 0, 110, 0, resolution);
      motor(i);
      }
    }
  h.sendhtml(body);
