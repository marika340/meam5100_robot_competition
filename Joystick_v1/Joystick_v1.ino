...
//SETUP FOR THE ESP32S2 TO BE THE HOTSPOT
// const char* ap_ssid = "Team8_Car";
// const char* ap_password = "pleasework";
...
uint32_t duty_cycle = 50; //PLACE HOLDER VALUE
...
float Kp = 1.4, Ki = 1.0, Kd = 0.0; //CHANGE FOR TUNING
float rpm_desired = 85.0; //CHANGE IF WANT
int gear_ratio = 34; //NEW MOTORS
float count_per_revolution = (12.0*4*gear_ratio); //12 CPR * 4 EDGES

//SET UP VARIABLES FOR MANUAL - PID TRANSITION
float current_manual_rpm[2] = {0,0};
bool auto_enable = false;
unsigned long manual_start_time = 0;
float pid_transition = 0.90; //HAND OFF TO PID TUNING WHEN RPM REACH 90% DESIRED
bool pid_enable = false;

//LOOP FOR ANALOG WRITE
void ledcAnalogWrite(uint8_t pin, uint32_t value, uint32_t value_max) {
  duty_cycle = map(value,0,value_max,0,resolution); //SET UP DUTY CYCLE
  // Set the PWM duty cycle of pin as a proportion of resolution bits
  ledcWrite(pin,duty_cycle); //DUTY CYCLE TO PWM
}

//ADD RPM HANDLE TO HANDLE FORCE STOP
void handleRPM() {
  rpm_desired = h.getVal(); // Update the target
  // Reset PID state to ensure a clean start later
  for(int i = 0; i < 2; i++) {
     integral[i] = 0; //RESET
     previous_error[i] = 0; //RESET
  }
  h.sendhtml(body);
}
...
void handleDir() {
  ...
  //WANT MOTOR TO STOP FIRST BEFORE CHANGING DIRECTION FOR GOOD PRACTICE
  motor_direction[0] = 0; motor_direction[1] = 0;
  motor(0); motor(1);
  delay(10); //WAIT FOR MOTORS TO STOP
...
void handleSpeed() {
  ...
  manual_start_time = millis(); //START TIMER TO COUNT FOR TRANSITION
  //FIRST START MANUAL MODE
  pid_enable = false;
  auto_enable = true; //TRANSITION TO AUTO MODE
  // CLEAR PID ERROR
  for(int i=0; i<2; i++) { 
    integral[i] = 0; 
    previous_error[i] = 0;
...
void handleDirectionLeft() {
  int dir = h.getVal();
  //STOP AND PAUSE MOTOR
  motor_direction[0] = 0;
  motor(0);
  delay(10);
  //SET DIRECTION
  motor_direction[0] = constrain(dir, -1, 1);
  integral[0] = 0; //CLEAR PID
...
void setup() {
  ...
  // // CONFIGURE ACCESS POINT FOR ESP32
  // Serial.println("Configuring Access Point...");
  // WiFi.mode(WIFI_AP);
  // WiFi.softAP(ap_ssid, ap_password);
  
  // // // // Disable sleep for lowest latency
  // WiFi.setSleep(false);
...
void autoMode() { //MANUAL MODE UP TO SPEED THEN START TUNING PID
  //HANDLE FORCE STOP BY RESETTING THE VALUES
...
//START PID TUNING WHEN MEET THE ELSE CONDITIONS
  if(pid_enable) {
    PIDcontrol();
  } else {
    ...
    //MAKE SURE THAT BOTH MOTORS ARE AT 90% DESIRED
    bool motor0_status = abs(current_manual_rpm[0]) >= abs(rpm_desired*pid_transition); 
    bool motor1_status = abs(current_manual_rpm[1]) >= abs(rpm_desired*pid_transition); 
    //IF MANUAL TAKES LONGER THAN 2 SECONDS, THEN FORCE PID
    bool timeout = (millis() - manual_start_time > 1500); //IF LONGER THAN 1.5 SECONDS, THEN DO PID
      if ((motor0_status && motor1_status) || timeout) {
        Serial.println(">>> TRANSITION: Desired RPM reached or Timeout. PID engaged.");
    //CLEAR PID ERROR WHEN TRANSITION
        for(int i=0; i<2; i++) { //MAKE SURE THE MOTOR TRANSITION SMOOTHLY INSTEAD OF DROP ALL THE WAY TO 0 WHEN KP KI KD VALUE CHANGES
          float current_error = abs(rpm_desired) - current_manual_rpm[i];
          if (Ki > 0) {
            float manual_pwm = map(constrain(abs(rpm_new),0,130),0,130,0,resolution);
            integral[i] = (manual_pwm - (Kp*current_error))/Ki;
          } else {
            integral[i] = 0;
          }
...
void handleAuto() {
  ...
  if(!auto_enable) {
      pid_enable = false;
      Serial.println("Auto Mode disabled by web request.");
  } else {
      Serial.println("Auto Mode enabled by web request.");
  }
  h.sendhtml(body);
}
void loop() {
  ...
  h.serve(); //WEB REQUEST
  autoMode(); //RUN THE NEW AUTO LOOP
  // //IF PID IS ON
  // if (pid_enable) {
  //   PIDcontrol();
  // } else {
