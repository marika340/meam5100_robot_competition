...
//=====================================================================
// TOPHAT PINS AND VARIABLES
// =====================================================================
int SDA_pin = 11;
int SCL_pin = 14;

int packetCounter = 0;
int packetsInLastWindow = 0;
unsigned long lastPacketWindow = 0;
...
void setup() {
  ...
  //I2C Initialization
  if (!Wire.begin(SDA_pin, SCL_pin, 40000)) {
    Serial.println(F("failed wire init"));
  }
...
void loop() {
  ...
  if (millis() - lastPacketWindow >= windowMs) {
    ...
    Wire.beginTransmission(0x28); //send packet to Tophat
    // Wire.write(packetCounter);
    // Wire.endTransmission();
    
    int b = Wire.requestFrom(0x28, (uint8_t)1);
    Serial.println(b);
    if (b == 1) {
      uint8_t health = Wire.read();
      Serial.println("health_val");
  ...
  //SHUT ROBOT DOWN IF HEALTH IS 0
  if (health == 0) {
    stopAllMotors();
    //Serial.println("Health 0 - Motors Stopped");
  }
